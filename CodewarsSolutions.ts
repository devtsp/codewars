...
const arrayDiff: Function = (a: number[], b: number[]): number[] => {
...
const getCount: Function = (str: string) => {
...
const openOrSenior: Function = (data: Array<[Number, Number]>): string[] => {
...
const binaryArrayToNumber: Function = (arr: number[]): number => {
...
const squareDigits: Function = (num: number): number => {
  const partial = [];
...
const duplicateEncode: Function = (word: string): string => {
...
const findShort: Function = (s: string): number => {
...
const reverseWords: Function = (str: string): string => {
  const result: string[] = [];
...
const narcissistic: Function = (value: number): boolean => {
...
const order: Function = (words: string): string => {
  return words
    .split(" ")
...
const comp: Function = (a1: number[] | null, a2: number[] | null): boolean => {
  if (a1 === null || a2 === null) return false;
  let rootsquaredArray: number[] = [];
  for (let i of a2) {
    rootsquaredArray.push(Math.sqrt(i))
  }
  const check: boolean = a1.sort((a, b) => a - b ).join() === rootsquaredArray.sort((a, b) => a - b ).join();
  return check;
};
